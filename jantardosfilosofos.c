...
void deixarGarfo(int nfilo)
{
	...
	teste(esquerda); //ve se o vizinho da esquerda pode comer agora.
	teste(direita);  //ve se o vizinho da direita pode comer agora.
...
int main()
{
	...
	for (i = 0; i < N; i++)
		...
		pthread_join(thread_id[i], NULL); //onde ocorre a sincronizacao
}